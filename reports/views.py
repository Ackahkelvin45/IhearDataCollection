    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            if datatype_ids:
                ...
                if ids_to_process:
                    categories = (
                        Category.objects.filter(data_type_id__in=ids_to_process)
                        .order_by("name")
                        .distinct()
                    )
                    return JsonResponse(
                        list(categories.values("id", "name")), safe=False
                    )
                ...
                if ids_to_process:
                    classes = (
                        Class.objects.filter(category_id__in=ids_to_process)
                        .order_by("name")
                        .distinct()
                    )
                ...
                if ids_to_process:
                    subclasses = (
                        SubClass.objects.filter(parent_class_id__in=ids_to_process)
                        .order_by("name")
                        .distinct()
                    )
                    return JsonResponse(
                        list(subclasses.values("id", "name")), safe=False
                    )
                ...
                if ids_to_process:
                    communities = (
                        Community.objects.filter(region_id__in=ids_to_process)
                        .order_by("name")
                        .distinct()
                    )
                    return JsonResponse(
                        list(communities.values("id", "name")), safe=False
                    )
    ...
    def _parse_ids(self, ids_string):
        ...
        try:
            if "," in str(ids_string):
                id_list = [
                    int(id_str.strip())
                    for id_str in str(ids_string).split(",")
    ...
    def post(self, request, *args, **kwargs):
        ...
        if payload["recordingDevice"]:
            queryset = queryset.filter(
                recording_device__icontains=payload["recordingDevice"]
            )
        ...
        durations = AudioFeature.objects.filter(noise_dataset__in=queryset).aggregate(
            ...
            total_duration=Round(
                ExpressionWrapper(Sum("duration") / 3600.0, output_field=FloatField()),
                2,
            ),
        )
