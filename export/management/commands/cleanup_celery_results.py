    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if redis_url.startswith('redis://') or redis_url.startswith('rediss://'):
                ...
                url_parts = redis_url.replace('redis://', '').replace('rediss://', '').split('/')
                ...
                try:
                    ...
                    if result_data:
                        ...
                        if ttl == -1:
                            if dry_run:
                                self.stdout.write(
                                    self.style.WARNING(f'Would delete: {key.decode("utf-8", errors="ignore")}')
                                )
                            else:
                                r.delete(key)
                                deleted_count += 1
                            ...
                            if dry_run:
                                self.stdout.write(
                                    self.style.WARNING(f'Would delete (expires soon): {key.decode("utf-8", errors="ignore")}')
                                )
                            else:
                                r.delete(key)
                                deleted_count += 1
                        ...
                        if dry_run:
                            self.stdout.write(
                                self.style.WARNING(f'Would delete (empty): {key.decode("utf-8", errors="ignore")}')
                            )
                        else:
                            r.delete(key)
                            deleted_count += 1
