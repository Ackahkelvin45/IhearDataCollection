...
class DatasetService:
    ...
    def query_dataset(self, question: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """
    ...
    def _handle_numeric_query(self, question: str, context: Dict = None) -> Dict[str, Any]:
        """
        ...
        counting_keywords = ['how many', 'how much', 'count', 'number of', 'total', 'amount of']
        statistical_keywords = ['average', 'mean', 'median', 'min', 'max', 'minimum', 'maximum', 'std', 'standard deviation', 'variance', 'distribution', 'statistics', 'stats', 'summary', 'analysis']
        dataset_keywords = ['dataset', 'datasets', 'datssets', 'datsset', 'document', 'documents', 'doc', 'docs', 'file', 'files', 'collection', 'collections', 'recording', 'recordings', 'audio']

        has_counting = any(keyword in question_lower for keyword in counting_keywords)
        has_statistics = any(keyword in question_lower for keyword in statistical_keywords)
        ...
        audio_features = ['mfcc', 'spectral', 'frequency', 'duration', 'sample rate', 'rms', 'energy', 'chroma', 'feature', 'features']
        has_audio_features = any(feature in question_lower for feature in audio_features)

        if (has_counting and has_dataset) or (has_statistics and has_dataset) or (has_audio_features):

            try:
                from data.models import Dataset as AudioDataset, NoiseDataset, AudioFeature, NoiseAnalysis
                ...
                if len(question_lower.split()) > 3:  # Complex questions with more context
                    llm_result = self.handle_complex_database_query(question)
                    if llm_result and llm_result['data_used']['type'] == 'llm_generated_sql':
                        return llm_result
                ...
                if has_audio_features or has_statistics:
                    ...
                    return self._handle_statistical_questions(question_lower, has_audio_features)
                else:
    ...
    def _handle_counting_questions(self):
        ...
        total_chunks = Document.objects.filter(processed=True).aggregate(
        ...
        if total_docs > 0:
            doc_info = f"You have {total_docs} document datasets uploaded for chatbot analysis"
            if processed_docs > 0:
        ...
        if total_audio_datasets > 0 or total_noise_records > 0:
            ...
            if total_noise_records > 0:
                audio_info += f"with {total_noise_records} individual noise recordings collected"
            audio_info += "."
    ...
    def _get_average_statistics(self):
        ...
        try:
            ...
            answer += f"• Average duration: {avg_stats['avg_duration']:.2f} seconds\\n" if avg_stats['avg_duration'] else "• Average duration: Not available\\n"
            answer += f"• Average sample rate: {avg_stats['avg_sample_rate']:.0f} Hz\\n" if avg_stats['avg_sample_rate'] else "• Average sample rate: Not available\\n"
            answer += f"• Average RMS energy: {avg_stats['avg_rms_energy']:.4f}\\n" if avg_stats['avg_rms_energy'] else "• Average RMS energy: Not available\\n"
            answer += f"• Average spectral centroid: {avg_stats['avg_spectral_centroid']:.2f} Hz\\n" if avg_stats['avg_spectral_centroid'] else "• Average spectral centroid: Not available\\n"
    ...
    def _get_distribution_statistics(self, question_lower):
        ...
        try:
            ...
            if 'region' in question_lower:
                regions = NoiseDataset.objects.values('region__name').annotate(count=Count('id')).order_by('-count')
                if regions:
                    answer += "📍 **By Region:**\\n"
            ...
            if 'category' in question_lower or 'breakdown' in question_lower:
                categories = NoiseDataset.objects.values('category__name').annotate(count=Count('id')).order_by('-count')
                if categories:
                    answer += "📂 **By Category:**\\n"
            ...
            if 'time' in question_lower or 'day' in question_lower:
                times = NoiseDataset.objects.values('time_of_day__name').annotate(count=Count('id')).order_by('-count')
                if times:
            ...
            if 'community' in question_lower:
                communities = NoiseDataset.objects.values('community__name').annotate(count=Count('id')).order_by('-count')
                if communities:
    ...
    def _get_audio_feature_statistics(self, question_lower):
        ...
        try:
            ...
            if basic_stats['avg_duration']:
                answer += f"⏱️ **Duration Statistics:**\\n"
                ...
                if basic_stats['min_duration']:
                    answer += f"• Shortest: {basic_stats['min_duration']:.2f} seconds\\n"
                answer += "\\n"
            ...
            if spectral_stats['avg_centroid']:
                ...
                if spectral_stats['avg_rolloff']:
                    answer += f"• Average rolloff: {spectral_stats['avg_rolloff']:.2f} Hz\\n"
                answer += "\\n"
    ...
    def _get_dataset_summary(self):
        ...
        try:
            ...
            # Top categories
            top_categories = NoiseDataset.objects.values('category__name').annotate(
                count=Count('id')
            ).order_by('-count')[:5]
            ...
            if top_categories:
                ...
                for category in top_categories:
                    if category['category__name']:
                        answer += f"• {category['category__name']}: {category['count']}\\n"
            ...
            for field in model._meta.get_fields():
                if hasattr(field, 'column'):
                    ...
                    if field.__class__.__name__ == 'ArrayField':
                        field_info['base_field_type'] = field.base_field.__class__.__name__
...
4. Use appropriate aggregations (COUNT, AVG, SUM, etc.)
5. Ensure the query is safe and won't modify data
...
SQL QUERY:"""
        ...
        try:
            ...
            # Validate the query is safe (only SELECT statements)
            if not sql_query.upper().startswith('SELECT') or 'UNABLE_TO_ANSWER' in sql_query:
                return None
    ...
    def _format_schema_for_prompt(self, schema_info):
        ...
        if schema_info['relationships']:
            ...
            for rel in schema_info['relationships']:
                formatted.append(f"  - {rel['from_table']}.{rel['from_field']} → {rel['to_table']}")
    ...
    def _analyze_sql_results(self, question_lower, sql_results):
        """Analyze SQL results and generate human-readable answer"""
        ...
        if sql_results['row_count'] == 0:
            return "I found no data matching your query in the database."

        # Generate human-readable summary based on results
        ...
        if len(columns) == 1 and len(rows) == 1:
            ...
            if len(values) <= 10:
                ...
                return f"I found {len(values)} results: {value_list}"
            else:
                return f"I found {len(values)} results. The first few are: {', '.join([str(v) for v in values[:5]])}..."
            ...
            return f"I found {len(rows)} records with {len(columns)} fields each. This is a complex query result. You may want to ask more specific questions to get detailed breakdowns."
    ...
    def _handle_explanatory_query(self, question: str, context: Dict = None) -> Dict[str, Any]:
        """
        ...
        if dataset_chunks:
            ...
            return {
                ...
                'data_used': {'type': 'explanatory_query', 'method': 'pattern_analysis'}
            }
    
    def _handle_mixed_query(self, question: str, context: Dict = None) -> Dict[str, Any]:
        """
