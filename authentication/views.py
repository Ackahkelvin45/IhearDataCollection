...
def resend_otp(request):
    ...
    if user_id:
        ...
        otp, email_result = send_otp(user)
        if email_result:
            messages.success(request, 'OTP has been resent to your email. Please verify to continue.')
            return redirect('auth:verify_otp')
    else:
...
def signup_view(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if email_result:
                request.session['otp_user_id'] = user.id
                messages.success(request, 'Account created! An OTP has been sent to your email. Please verify to continue.')
                return redirect('auth:verify_otp')
            else:
                messages.error(request, 'Account created but failed to send OTP. Please contact support.')
                return redirect('auth:login')
...
def verify_userotp(request):
    ...
    if request.method == 'POST':
        ...
        if is_valid:
            ...
            messages.success(request, 'OTP verified successfully! Please login with your credentials.')
            return redirect('auth:login')
        else:
