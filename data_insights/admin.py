...
class ChatMessageAdmin(admin.ModelAdmin):
    list_display = ["session_title", "status", "created_at"]
    list_filter = ["status", "created_at"]
    search_fields = [
        "user_input",
        "assistant_response",
    ...
    def session_link(self, obj):
        if obj.session:
            url = reverse("admin:data_insights_chatsession_change", args=[obj.session.id])
    ...
    def formatted_tool_call(self, obj):
        if obj.tool_called:
            try:
                formatted = json.dumps(obj.tool_called, indent=2)
                return format_html("<pre>{}</pre>", formatted)
            except:
                return str(obj.tool_called)
        return "-"
