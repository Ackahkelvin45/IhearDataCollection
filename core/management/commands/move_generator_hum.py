...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Old location
        old_category_name = "Industrial and construction sounds"
        old_class_name = "Machinery"
        old_subclass_name = "Generator hum"

        # New location
        new_category_name = "Urban Life and Public Spaces"
        ...
        try:
            with transaction.atomic():
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f'✗ Old category not found: {old_category_name}')
                    )
                    return
                ...
                if created:
                    self.stdout.write(
                        self.style.SUCCESS(f'✓ Created new category: {new_category_name}')
                    )
                else:
                ...
                if created:
                    self.stdout.write(
                        self.style.SUCCESS(f'✓ Created new subclass: {new_subclass_name}')
                    )
                else:
