...
def load_audio_file(audio_path: str, ext: str) -> Tuple[np.ndarray, int]:
    ...
    try:
        ...
        if ext in SUPPORTED_FORMATS["wav"] + SUPPORTED_FORMATS["flac"] + SUPPORTED_FORMATS["aiff"]:
            try:
        ...
        raise ValueError(f"Could not load audio file {audio_path} with any available backend")
...
def extract_basic_features(y: np.ndarray, sr: int) -> dict:
    ...
    try:
        ...
        # Spectral bandwidth
        spectral_bandwidth = np.sqrt(np.sum(((pos_freqs - spectral_centroid)**2) * pos_fft) / np.sum(pos_fft)) if np.sum(pos_fft) > 0 else 0
...
def create_chroma_features(freqs: np.ndarray, magnitudes: np.ndarray, sr: int) -> np.ndarray:
    """Create simplified chroma features"""
    # Define chroma frequencies (C, C#, D, D#, E, F, F#, G, G#, A, A#, B)
    chroma_freqs = np.array([261.63, 277.18, 293.66, 311.13, 329.63, 349.23, 
