...
class AudioFeature(models.Model):
    ...
    # Visualization data
    mel_spectrogram = models.JSONField(
        help_text="Mel spectrogram data"
        ,null=True
    )
    waveform_data = models.JSONField(
        help_text="Waveform data for visualization"
        ,null=True
    )
...
class NoiseAnalysis(models.Model):
    ...
    std_db = models.FloatField(help_text="Standard deviation of decibel levels",null=True)
    
    # Temporal features
    peak_count = models.IntegerField(help_text="Number of significant peaks",null=True)
    peak_interval_mean = models.FloatField(help_text="Mean interval between peaks",null=True)
    
    # Frequency analysis
    dominant_frequency = models.FloatField(help_text="Dominant frequency in Hz",null=True)
    frequency_range = models.CharField(
        max_length=100,
        help_text="Frequency range (low-high)",null=True
    )
    ...
    event_durations = ArrayField(
        models.FloatField(),
        help_text="Durations of detected events in seconds",null=True
    )
