...
class CleanSpeechDataset(models.Model):
    ...
    clean_speech_id = models.CharField(
        max_length=255,
        blank=True,
        null=True,
        unique=True,
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.id:
            ...
            self.dataset_type = clean_speech_dataset
        super().save(*args, **kwargs)
...
class VisualizationPreset(models.Model):
    ...
    clean_speech_dataset = models.ForeignKey(
        CleanSpeechDataset,
        on_delete=models.CASCADE,
        related_name="visualization_presets",
        null=True,
...
class CleanSpeechAudioFeature(models.Model):
    """Audio features extracted from clean speech recordings"""

    clean_speech_dataset = models.OneToOneField(
        CleanSpeechDataset, on_delete=models.CASCADE, related_name="audio_features"
    )
    ...
    def __str__(self):
        return f"Audio features for {self.clean_speech_dataset.clean_speech_id}"


class CleanSpeechAnalysis(models.Model):
    ...
    clean_speech_dataset = models.OneToOneField(
        CleanSpeechDataset, on_delete=models.CASCADE, related_name="analysis"
    )
